...
internal class HttpRequestAnalyzerTest {
    @LocalServerPort
    ...
    companion object {
        @JvmStatic
        fun analyzerTestCases(): Stream<Arguments> {
            val analyzerDir = File("src/test/resources/analyzer")
    ...
    @MethodSource("analyzerTestCases")
    @DisplayName("Analyzer Test Cases")
    @Throws(Exception::class)
    fun `test analyzer cases`(
        version: String,
        name: String,
        subDir: File,
    ) {
        ...
        val requestBody = AnalyzeRequest(version, code, rulesMap)
